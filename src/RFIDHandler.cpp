...
CMyApplication::CMyApplication(const char *pReaderHostName_t)
		: m_Verbose(0), m_pConnectionToReader(NULL)
{
	m_messageID = 0;
	m_channelIndex = 1;

	pReaderHostName = (char *)malloc(sizeof(char) * 128);
	strcpy(pReaderHostName, pReaderHostName_t);
	// std::signal(SIGINT, SignalHandlerWrapper);
	// init_keyboard();
	// std::thread t1(&CMyApplication::keyboardThread, this);
    // t1.join();
...
CMyApplication::~CMyApplication(void)
{
	int rc = stop();
	restore_keyboard();
	free(pReaderHostName);
}


void CMyApplication::init_keyboard(){
...
void CMyApplication::restore_keyboard(){
	
	tcsetattr(kfd, TCSANOW, &cooked);

}

void CMyApplication::keyboardThread(){
	char c;
	if (read(kfd, &c, 1) < 0)
	{
		perror("read():");
...
// void CMyApplication::SignalHandler() {
// 	printf("Terminal interrupt signal received.\n");
...
// void CMyApplication::SignalHandlerWrapper(int signal) { 
// 	// 将静态成员函数转发到非静态成员函数
					...
					if (0 == addROSpec())
					{
						...
						if (0 == enableROSpec())
						{
							...
							while (!flag)
							{
								...
								// 	}
								// 	pub1.publish(msg1);
								// 	printf("Istatic void SignalHandler(int signal) {NFO: publish msg1 of tag %d finished\n", count);
								// 	rate_20hz.sleep();
...
int CMyApplication::stop(){
    ...
    // }
    // else{
    //     printf("INFO: ROSpec failed\n");
    // }

    // int stopROS = stopROSpec();
    /* Clean the Reader */
    if(m_Verbose)
    {
        printf("INFO: Clean up reader configuration...\n");
	...
	CTagReportData *pTagReportData)
{
	...
	if (getOneTimestamp(pTagReportData->getLastSeenTimestampUTC(), &time))
	{
		...
		oneTagData.timestamp_pc = time;

		// int Msec = ros::Time::now().toNSec() / 1000000;

		// float sec = (float)Msec / 1000;

		// oneTagData.timestamp_ros = sec;

		oneTagData.timestamp_ros = ros::Time::now().toSec();
	}
    ...
    for(auto it = dataVec.begin(); it != dataVec.end(); it++){
        if(strcmp((*it).epc, oneTagData.epc) == 0){
            (*it).antennaId.push_back(oneTagData.antennaId);
            (*it).channelIndex.push_back(oneTagData.channelIndex);
            (*it).phase.push_back(oneTagData.phase);
            (*it).rssi.push_back(oneTagData.rssi);
            (*it).timestamp_pc.push_back(oneTagData.timestamp_pc);
            (*it).timestamp_ros.push_back(oneTagData.timestamp_ros);
            flag = true;
            break;
        }
    }
    ...
    if(!flag){
        RFIDData data;
        strcpy(data.epc, oneTagData.epc);
        data.antennaId.push_back(oneTagData.antennaId);
        data.channelIndex.push_back(oneTagData.channelIndex);
        data.phase.push_back(oneTagData.phase);
        data.rssi.push_back(oneTagData.rssi);
        data.timestamp_pc.push_back(oneTagData.timestamp_pc);
        data.timestamp_ros.push_back(oneTagData.timestamp_ros);
        dataVec.push_back(data);
    }
}

void CMyApplication::saveTag2Buff(void){
    while (isRunning) // ��ѭ��
	{
